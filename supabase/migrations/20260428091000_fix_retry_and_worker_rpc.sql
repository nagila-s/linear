-- Correcoes de retry + RPC worker v2.
...
BEGIN
  ...
  IF v_status_type IS NOT NULL THEN
    EXECUTE format('ALTER TYPE %I ADD VALUE IF NOT EXISTS ''retrying''', v_status_type);
  END IF;
END $$;
...
BEGIN
  ...
  UPDATE public.jobs
  SET
    status = CASE WHEN v_attempts < v_max THEN 'retrying' ELSE 'failed' END,
